...
class MockmanProvider {
  ...
  async refresh() {
    ...
    try {
      const res = await axios.get(`https://api.mockman.online/collections/${apiKey}`, { timeout: 5000 });
      this.collections = res.data || [];
    } catch (error) {
      vscode.window.showErrorMessage(`Error fetching collections: ${error.message}`);
      this.collections = [];
    }
...
class TemplateBrowserWebview {
      ...
      async (message) => {
        ...
        switch (message.command) {
          ...
          case 'getTemplates':
            ...
            try {
              const templateRes = await axios.get(`https://api.mockman.online/templates/${apiKey}`, { timeout: 5000 });
              this.panel.webview.postMessage({ command: 'templates', data: templateRes.data.templates });
            } catch (error) {
          ...
          case 'getPreview':
            ...
            try {
              const previewRes = await axios.get(`https://api.mockman.online/templates/${apiKey}/${message.category}/preview`, { timeout: 5000 });
              this.panel.webview.postMessage({ command: 'preview', data: previewRes.data });
            } catch (error) {
          ...
          case 'createCollection':
            ...
            try {
              const createRes = await axios.post(`https://api.mockman.online/templates/${apiKey}/create`, {
                category: message.category,
                count: message.count
              }, { timeout: 10000 });
              this.panel.webview.postMessage({ command: 'createSuccess', data: createRes.data });
              vscode.window.showInformationMessage(`✅ Collection "${createRes.data.collection.name}" created with ${createRes.data.collection.documentCount} documents`);
  ...
  showLoginError() {
    ...
    this.panel.webview.postMessage({ command: 'error', message: 'Please login first (MockMan: Login).' });
  }
...
async function activate(context) {
      ...
      handleUri(uri) {
        if (uri.path === '/callback') {
          ...
          if (apiKey && apiKey.length > 0 && /^[a-f0-9]+$/.test(apiKey)) {
            context.secrets.store('mockman.apiKey', apiKey.trim())
              .then(() => {
                provider.apiKey = apiKey.trim();
                return vscode.workspace.getConfiguration('mockman').update('apiKey', apiKey.trim(), vscode.ConfigurationTarget.Global);
              })
              .then(() => {
              ...
              .catch((error) => {
                vscode.window.showErrorMessage('⚠️ Failed to save API Key: ' + error.message);
              });
          } else {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('mockman.login', async () => {
      ...
      if (!success) {
        vscode.window.showWarningMessage(`Could not open browser. Please visit ${loginUrl} manually.`, 'Copy URL')
          .then(selection => {
    ...
    vscode.commands.registerCommand('mockman.showEndpoints', async (collection) => {
      if (!collection) {
